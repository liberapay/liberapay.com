...
class TestCommunitiesJson(Harness):
    ...
    def test_joining_and_leaving_community(self):
        response = self.client.POST('/alice/communities.json', {'do': 'join:'+self.c_id}, auth_as=self.alice, xhr=True
                                    )

        r = json.loads(response.body)
        assert r == {}

        response = self.client.POST('/alice/communities.json', {'do': 'leave:'+self.c_id}, auth_as=self.alice, xhr=True
                                    )
