...
class TestCronJobs(EmailHarness):
    ...
    def test_profile_description_missing_notification(self):
        ...
        # fast forward 7 months, bob should get another notification,
        # but ethan shouldn't because he left the team
        self.db.run("UPDATE notifications SET ts = ts - interval '7 months'")
        generate_profile_description_missing_notifications()
        emails = self.get_emails()
        assert len(emails) == 1
        assert emails[0]['to'][0] == 'bob <%s>' % bob.email
